...
export default function ConfirmAnswer(props) {
    const { answerInput, handleConfirmClose, handleSubmitBtn, popupDivTw } =
        props
    ...
    let confirmAnswer = (
        ...
        >
            <p className="w-full mb-[10px]">You answered:</p>
    ...
    let confirmCancelBtn = (
        <div key={`confirm-cancel-div`} className="w-full">
            <button onClick={handleConfirmClose} className={`${cancelBtnTw}`}>
    ...
    const confirmh3 = (
        <h3 key={`confirmAnswer-h3`} className={h3Tw}>
            Confirm Answer
        </h3>
    )
